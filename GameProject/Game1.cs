        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < bears.Count; i++)
            {
                for (int j = i + 1; j < bears.Count; j++)
                {
                    ...
                    {
                      CollisionResolutionInfo collisionHappened = CollisionUtils.CheckCollision(gameTime.ElapsedGameTime.Milliseconds,
                        ...
                        if (collisionHappened != null)
                        {
                            if (collisionHappened.FirstOutOfBounds == true)
                            {
                                bears[i].Active = false;
                            }
                            else
                            {
                                bears[i].Velocity = collisionHappened.FirstVelocity;
                                bears[i].DrawRectangle = collisionHappened.FirstDrawRectangle;
                                teddyBounce.Play();
                            }

                            if (collisionHappened.SecondOutOfBounds == true)
                            {
                                bears[j].Active = false;
                            }
                            else
                            {
                                bears[j].Velocity = collisionHappened.SecondVelocity;
                                bears[j].DrawRectangle = collisionHappened.SecondDrawRectangle;
                                teddyBounce.Play();
                            }
                        }
                    }
            ...
            foreach (TeddyBear bear in bears)
            {
                if (bear.Active == true && bear.CollisionRectangle.Intersects(burger.CollisionRectangle))
                {
                    ...
                    burgerDamage.Play();
                    bear.Active = false;
                    Explosion boom = new Explosion(explosionSpriteStrip, 
